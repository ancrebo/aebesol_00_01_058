...
numepoch = 50000
unitslstm1 = 50
namesave = 'LSTM_'+str(unitslstm1)+'N_'+str(numepoch)+'epochmodel1'
...
datatest1 = dataval_tot1[0:len(dataval_tot1):1,:]
# testx1 = datatest1[:,6:]
# test_cl1 = dataval_tot1[0:len(dataval_tot1):1,0]
# test_clst1 = dataval_tot1[0:len(dataval_tot1):1,1]
# test_deltacl1 = dataval_tot1[0:len(dataval_tot1):1,2]
# test_cm1 = dataval_tot1[0:len(dataval_tot1):1,3]
# test_cmst1 = dataval_tot1[0:len(dataval_tot1):1,4]
# test_deltacm1 = dataval_tot1[0:len(dataval_tot1):1,5]
...
# data_totb = data_tot.copy()
# data_norm = norm(data_tot,data_totb)
# data_norm = data_norm.fillna(0)
# data_norm['Flag'] = flagout
# dataval_tot = data_norm.values

column_names = ["CL","CL_st","Delta CL","CM","CM_st","Delta CM","Alpha0","Delta Alpha","Alpha prima","Alpha prima prima","Flag"]
data_norm1t = norm(data_tot,data_tot1b)
data_norm1t = data_norm1t.fillna(0)
data_norm1t = data_norm1t.reindex(columns=column_names)
data_norm1t['Flag'] = flagout
...
datatest1t = dataval_tot1t[0:len(dataval_tot1t):1,:]
# testx1t = datatest1t[:,6:]
# test_cl1t = dataval_tot1t[0:len(dataval_tot1t):1,0]
# test_clst1t = dataval_tot1t[0:len(dataval_tot1t):1,1]
# test_deltacl1t = dataval_tot1t[0:len(dataval_tot1t):1,2]
# test_cm1t = dataval_tot1t[0:len(dataval_tot1t):1,3]
# test_cmst1t = dataval_tot1t[0:len(dataval_tot1t):1,4]
# test_deltacm1t = dataval_tot1t[0:len(dataval_tot1t):1,5]
...
trainx1t = np.reshape(trainx1t,(trainx1t.shape[0],1,trainx1t.shape[1]))

mat_train1t = np.zeros((len(train_deltacl1t),2))
mat_train1t[:,0] = train_deltacl1t
mat_train1t[:,1] = train_deltacm1t
# testx1t = np.reshape(testx1t,(testx1t.shape[0],1,testx1t.shape[1]))
# #%%
# trainx = dataval_tot[0:len(dataval_tot):1,6:]
# train_cl = dataval_tot[0:len(dataval_tot):1,0]
# train_clst = dataval_tot[0:len(dataval_tot):1,1]
# train_deltacl = dataval_tot[0:len(dataval_tot):1,2]
# train_cm = dataval_tot[0:len(dataval_tot):1,3]
# train_cmst = dataval_tot[0:len(dataval_tot):1,4]
# train_deltacm = dataval_tot[0:len(dataval_tot):1,5]
# datatest = dataval_tot[0:len(dataval_tot):1,:]
# testx = datatest[:,6:]
# test_cl = dataval_tot[0:len(dataval_tot):1,0]
# test_clst = dataval_tot[0:len(dataval_tot):1,1]
# test_deltacl = dataval_tot[0:len(dataval_tot):1,2]
# test_cm = dataval_tot[0:len(dataval_tot):1,3]
# test_cmst = dataval_tot[0:len(dataval_tot):1,4]
# test_deltacm = dataval_tot[0:len(dataval_tot):1,5]

# mat_train = np.zeros((len(train_deltacl),2))
# mat_train[:,0] = train_deltacl
# mat_train[:,1] = train_deltacm

# dimtemp = trainx.shape[0]
# dimvar = trainx.shape[1]
...
#%%
val_trainx1t = np.zeros((dimtemp1,1,dimvar1))
...
model1 = Sequential()
model1.add(LSTM(unitslstm1,  batch_input_shape=(dimtemp1,1, dimvar1)))
model1.add(Dense(2))
weights = model1.layers[0].get_weights()
weights[0][4,:]=-10E30
model1.layers[0].set_weights(weights)
...
#    test_clcm = model1.predict(val_trainx1t)
#    test_clcm = test_clcm[:dimtemp1t,:]
#    test_clcmreal = val_mat_train1t[:dimtemp1t,:]
#    loss_train[ii] = np.mean(sum((mat_train1-train_clcm)**2)/dimtemp1)
#    loss_test[ii] = np.mean(sum((test_clcmreal-test_clcm)**2)/dimtemp1t)
#    history1 = model1.fit(trainx1, mat_train1, epochs=1,batch_size=dimtemp1,verbose=0)
...
#dimtemp = 1
model1b = Sequential()
model1b.add(LSTM(unitslstm1,  batch_input_shape=(len(trainx1),1, dimvar)))
model1b.add(Dense(2))
...
alpha0_case1 = trainx1[:,0,0]*(np.amax(data_tot1b["Alpha0"])-np.amin(data_tot1b["Alpha0"]))+np.amin(data_tot1b["Alpha0"])
deltaalpha_case1 = trainx1[:,0,1]*(np.amax(data_tot1b["Delta Alpha"])-np.amin(data_tot1b["Delta Alpha"]))+np.amin(data_tot1b["Delta Alpha"])
...
#%% Se carga  la red para nuevos datos
weights0 = model1.layers[0].get_weights()
weights1 = model1.layers[1].get_weights()
model1t = Sequential()
model1t.add(LSTM(unitslstm1,  batch_input_shape=(dimtemp1t,1, dimvar1t)))
model1t.add(Dense(2))
...
CL_nn = np.zeros((dimtemp1,1))
CM_nn = np.zeros((dimtemp1,1))
