...
tperiod = 1/((k/3.933333e-04)**0.5/2/np.pi)
...
# c = 0.1
# cfdseguir = pd.DataFrame()
# cfdmotion = pd.read_csv('data/seguirmotion/postproc_a0_03_0_A_02_50_f_50_00.csv')
...
# alphamed = np.zeros((len(cfdseguir),))
# deltaalpha = np.zeros((len(cfdseguir),))
# tperiod = 1/50 #1/((18.9464/3.933333e-04)**0.5/2/np.pi)
# num = int(tperiod/delta_t)
# print("aoamed")
#%%
...
datasave["theta"] = cfdseguir["theta"]
datasave["w"] = cfdseguir["w"]
datasave["alpha"] = cfdseguir["alpha"]
datasave.to_csv("seguir/k_12_5_LSTM"+str(unitslstm)+"_FNN"+str(unitsfnn)+".csv")
