...
#%% Structural Model
kadim_vec_st = np.array([20])#  np.array([3,5.5,6.5,8.5,12.5,20,50,90])#3,4,5,6,7,8,9,12,21,80[5,7,20,80]np.array([470,700,1000,1300,1600,1920,2000,3300,13480])*1e6
...
for jj in np.arange(len(kadim_vec_st)):
    ...
    for ii in np.arange(numtime-1):
        ...
        if flag_in == 0:
            if np.abs(aoa_vec[ii]) < 60:
                cl_st[ii] = f_cl_st(aoa_vec[ii])
                cm_st[ii] = -f_cm_st(aoa_vec[ii])
    ...
    torsionmean_st[jj] = np.mean(theta_vec[-int(nmean/2):])
    bendingmean_st[jj] = np.mean(w_vec[-int(nmean/2):])
    torsionmax_st[jj] = np.max(theta_vec[-int(nmean/2):])
    bendingmax_st[jj] = np.max(w_vec[-int(nmean/2):])
    torsionmin_st[jj] = np.min(theta_vec[-int(nmean/2):])
    bendingmin_st[jj] = np.min(w_vec[-int(nmean/2):])



#%% Structural Model
kadim_vec_fnn =  np.array([6.5])#np.array([3,5.5,6.5,8.5,12.5,20,50,90])#3,4,5,6,7,8,9,12,21,80[5,7,20,80]np.array([470,700,1000,1300,1600,1920,2000,3300,13480])*1e6
...
for jj in np.arange(len(kadim_vec_fnn)):
    ...
    torsionmean_fnn[jj] = np.mean(theta_vec[-int(nmean/2):])
    bendingmean_fnn[jj] = np.mean(w_vec[-int(nmean/2):])
    torsionmax_fnn[jj] = np.max(theta_vec[-int(nmean/2):])
    bendingmax_fnn[jj] = np.max(w_vec[-int(nmean/2):])
    torsionmin_fnn[jj] = np.min(theta_vec[-int(nmean/2):])
    bendingmin_fnn[jj] = np.min(w_vec[-int(nmean/2):])

#%% Structural Model
kadim_vec =  np.array([6.5])#np.array([3,5.5,6.5,8.5,12.5,20,50,90])#3,4,5,6,7,12,21,80[5,7,20,80]np.array([470,700,1000,1300,1600,1920,2000,3300,13480])*1e6
...
for jj in np.arange(len(kadim_vec)):
    ...
    torsionmean[jj] = np.mean(theta_vec[-int(nmean/2):])
    bendingmean[jj] = np.mean(w_vec[-int(nmean/2):])
    torsionmax[jj] = np.max(theta_vec[-int(nmean/2):])
    bendingmax[jj] = np.max(w_vec[-int(nmean/2):])
    torsionmin[jj] = np.min(theta_vec[-int(nmean/2):])
    bendingmin[jj] = np.min(w_vec[-int(nmean/2):])
...
with open('data/workspace/lstm'+str(unitslstm)+'Nepoch'+str(numepochlstm)+'N_FNN'+str(unitsfnn)+'Nepoch'+str(numepochfnn)+'mma.pkl', 'wb') as f:
    pickle.dump([data_sum,timevec,data_kref,data_kref_st,data_kref_fnn,datacfd_kref,kref_vec,torsionmean,kref_vec_fnn,torsionmean_fnn,kref_vec_st,torsionmean_st,kadimcfd,mean_theta_cfd], f)