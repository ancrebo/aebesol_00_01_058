...
for f in [f_ for f_ in os.listdir(dataset_path_base) if 'a0' in f_]:
    ...
    if  data["Amplitude Monitor: Expression"].values[-1]*180/np.pi<5 and  data["Alpha0 Monitor: Expression"].values[-1]*180/np.pi<7 and freq>30 and freq<70:
...
cfdmotion = pd.read_csv('data/seguirmotion/mov_k_8.csv')
...
trainPredict1 = model1fnnb.predict(data2fnn)
